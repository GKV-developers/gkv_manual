# Supplemental {#chap:supplemental}

## Entropy balance equation for each wavenumber and plasma species {#sec:entropy-balance-equation-for-each-wavenumber-and-plasma-species}
...
\begin{align}
  ...
  \frac{dW_{\mathrm{M}\bm{k}}}{dt} &= - \sum_\mathrm{s} R_{\mathrm{sM}\bm{k}},\end{align}
...
\begin{align}
  ...
  E_{\mathrm{s}\bm{k}} &= \mathrm{Re} \left[ - \left\langle \int dv^3 v_\parallel \nabla_\parallel \frac{T_\mathrm{s}|\tilde{g}_{\mathrm{s}\bm{k}}|^2}{2F_\mathrm{sM}} \right\rangle \right],\end{align}
...
\begin{align}
  ...
  \tilde{q}_{\parallel\mathrm{s}\bm{k}} &= \tilde{q}_{\parallel\parallel\mathrm{s}\bm{k}} + \tilde{q}_{\parallel\perp\mathrm{s}\bm{k}}.\end{align}

See also Refs.
[Sugama2009PoP](#bib:Sugama2009PoP){.bib-ref data-bib-key="Sugama2009PoP"} and [Maeyama2014PoP](#bib:Maeyama2014PoP){.bib-ref data-bib-key="Maeyama2014PoP"}.

## Triad transfer function {#sec:triad-transfer-function}

\begin{align}
  J_{\mathrm{s}\bm{k}}^{\bm{p},\bm{q}} &= \delta_{\bm{k}+\bm{p}+\bm{q},\bm{0}} \frac{\bm{b} \cdot \bm{p} \times \bm{q}}{2c_b} \mathrm{Re} \left[ \left\langle \int dv^3 (\chi_{\mathrm{s}\bm{p}} \tilde{g}_{\mathrm{s}\bm{q}} - \chi_{\mathrm{s}\bm{q}} \tilde{g}_{\mathrm{s}\bm{p}}) \frac{T_\mathrm{s}\tilde{g}_{\mathrm{s}\bm{k}}}{F_\mathrm{sM}} \right\rangle \right], \end{align}

where
$\tilde{g}_{\mathrm{s}\bm{k}} = \tilde{f}_{\mathrm{s}\bm{k}} + e_\mathrm{s} J_{0\mathrm{s}\bm{k}} \tilde{\phi}_{\bm{k}} F_\mathrm{sM} / T_\mathrm{s}$
and
$\chi_{\mathrm{s}\bm{k}} = J_{0\mathrm{s}\bm{k}} (\tilde{\phi}_{\bm{k}} - v_\parallel \tilde{A}_{\parallel\bm{k}})$.
The triad transfer function satisfy the following properties
[Nakata2012PoP](#bib:Nakata2012PoP){.bib-ref data-bib-key="Nakata2012PoP"}: 

\begin{align}
  &J_{\mathrm{s}\bm{k}}^{\bm{p},\bm{q}} = J_{\mathrm{s}\bm{k}}^{\bm{q},\bm{p}}, \\
  &J_{\mathrm{s}\bm{k}}^{\bm{p},\bm{q}} + J_{\mathrm{s}\bm{p}}^{\bm{q},\bm{k}} + J_{\mathrm{s}\bm{q}}^{\bm{k},\bm{p}} = 0.\end{align}

Note that $J_{\mathrm{s}\bm{k}}^{\bm{p},\bm{q}}$ is symmetrized so as to
eliminate asymmetric components, which cancel out in the net entropy
transfer $I_{\mathrm{s}\bm{k}}$ and thus do not contribute to physics.
Since the terms of $\tilde{\phi}_{\bm{k}}$ and of
$\tilde{A}_{\parallel\bm{k}}$ respectively correspond to
$\bm{E}\times\bm{B}$ and magnetic flutter nonlinearities, these
...
\begin{align}
  ...
  J_{\mathrm{s}\bm{k}}^{\bm{p},\bm{q}} &= J_{\mathrm{sE}\bm{k}}^{\bm{p},\bm{q}} + J_{\mathrm{sM}\bm{k}}^{\bm{p},\bm{q}}.\end{align}

## Integrals in GKV {#sec:integrals-in-gkv}

Flux-surface average: 

\begin{align}
  &\left\langle \tilde{\phi} (x,y,z) \right\rangle = \sum_{k_x} \left\langle \tilde{\phi}_{k_x,k_y=0}(z) \right\rangle e^{ik_xx}, \\
  &\left\langle \tilde{\phi}_{k_x,k_y=0}(z) \right\rangle = \frac{\int_\pi^\pi dz \sqrt{g} \tilde{\phi}_{k_x,k_y=0}(z)}{\int_\pi^\pi dz \sqrt{g}}.\end{align}

Volume average: 

\begin{align}
  \int dx^3 \left|\tilde{\phi}(x,y,z)\right|^2 = \sum_{k_x} \sum_{k_y} \left\langle \left| \tilde{\phi}_{\bm{k}} (z) \right|^2 \right\rangle.\end{align}

Velocity-space integral: 

\begin{align}
  \int dv^3 \tilde{f}_{\mathrm{s}\bm{k}}(z,v_\parallel,mu) = \int_{-L_v}^{L_v}dv_\parallel \int_0^{L_v} dv_\perp 2\pi v_\perp \tilde{f}_{\mathrm{s}\bm{k}}(z,v_\parallel,\mu).\end{align}

## References {#sec:references}

<div class="thebibliography">

<span class="bibitem" id="bib:Sugama2009PoP" data-bib-key="Sugama2009PoP">
H. Sugama, T.-H. Watanabe, and M. Nunami, <em>Phys. Plasmas</em> <strong>16</strong>, 112503 (2009).
</span><br>

<span class="bibitem" id="bib:Maeyama2014PoP" data-bib-key="Maeyama2014PoP">
S. Maeyama, A. Ishizawa, T.-H. Watanabe, M. Nakata, N. Miyato, M. Yagi, and Y. Idomura, <em>Phys. Plasmas</em> <strong>21</strong>, 052301 (2014).
</span><br>

<span class="bibitem" id="bib:Nakata2012PoP" data-bib-key="Nakata2012PoP">
M. Nakata, T.-H. Watanabe, and H. Sugama, <em>Phys. Plasmas</em> <strong>19</strong>, 022303 (2012).
</span>
