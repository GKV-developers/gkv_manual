.. _sec_adiabatic-electron-ion-model-for-nprocs-1:
...
electrostatic limit is assumed (:math:`\tilde{A}_\parallel = 0`), and
...
The temperature ratio :math:`T_\mathrm{e}/T_\mathrm{i}` is given by ``tau_ad``
...
where :math:`\langle \cdots \rangle` denotes the flux-surface average, and
:math:`\delta_{i,j}` is the Kronecker's delta. Density, temperature and mass
...
The temperature ratio :math:`T_\mathrm{i}/T_\mathrm{e}` is given by ``tau_ad``
in ``run/gkvp_namelist``.
