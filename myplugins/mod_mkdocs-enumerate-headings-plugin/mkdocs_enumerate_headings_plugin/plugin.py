...
class EnumerateHeadingsPlugin(BasePlugin):
    config_scheme = (
        ...
        ("mod_appendix_next", config_options.Type(str, default="")),
    )
    ...
    def on_nav(self, nav, config, files, **kwargs):
        ...
        """
        def set_chapter_numbers(items, parent_chapters: list[int] = [], is_appendix=False):
            """
            Set chapter numbers for each pages.
            Counter of Pages under chapter is same.
            if the section is below, `1 Chap1`, `2.1 sub 1`, `2.2 sub 2`, `A.1 title 1`, `A.2 title 2`
            - Chap 1.md (1)
            - Chap 2
              - sub 1.md (2.1)
              - sub 2.md (2.2)
            ...
            - supplemental (B)
            """
            chapter_counter = parent_chapters + [0]
            ...
            for i, item in enumerate(items):
                if item.is_page:
                    ...
                    if page.file.abs_src_path not in markdown_files_processed:
                        chapter = chapter_counter.copy()
                        chapter[-1] += 1
                        markdown_files_processed[page.file.abs_src_path] = chapter
                        chapter_counter[-1] += page.number_h1s
                    else:
                        chapter = markdown_files_processed[page.file.abs_src_path].copy()

                    page.chapter = chapter

                    page.is_appendix = is_appendix
                    # after last of main contents, set parameters for appendix
                    mod_appendix_next = os.path.normpath(self.config.get("mod_appendix_next", ""))
                    if page.file.src_path == mod_appendix_next:
                        chapter_counter[-1] = 0
                        is_appendix = True

                elif item.is_section:
                    if item.title not in markdown_files_processed:
                        chapter = chapter_counter.copy()
                        chapter[-1] += 1
                        markdown_files_processed[page.title] = chapter
                        chapter_counter[-1] += 1
                    else:
                        chapter = markdown_files_processed[page.title].copy()
                    set_chapter_numbers(item.children, chapter_counter, is_appendix)
        set_chapter_numbers(nav.items, [], False)
    ...
    def on_post_page(self, output, page, config, **kwargs):
        ...
        # Process HTML
        htmlpage = HTMLPage(output, is_appendix=page.is_appendix)
